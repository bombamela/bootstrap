...
parser.add_argument('--outpipe', required=True)
...
def detect(doTrack):
    ...
    video_capture, test_list = read_frame(video_capture, data_mean, capture, frameCounter)
    ...
    while True:
        ...
        video_capture, inputs = read_frame(video_capture, data_mean, capture, frameCounter)
        ...
        if display:
            for rect in acc_rects:
                ...
                BB = np.array([[rect.cx - int(rect.width / 2),
        ...
        frame = doTrack.displayTrackingResults(frameName, step, img, args.outpipe, frameCounter, delay)
        t3 = time.clock()
        print 'TIMING: All:%f D:%f T:%f L:%f\n' % (t3 - t0, t3 - t2, t2 - t1, t1 - t0)


        capture_manager = CaptureManager(video_capture, True)
